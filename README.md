...
**Открытое ядро для Telegram и Discord.** Свои функции пишете локально и подключаете через `SB_MODULES`; в этот GitHub они не попадают. В комплекте — примеры модулей (предложка, мост, модерация и др.).
...
**Suggest Bridge** — открытое ядро для Telegram и Discord. Ядро `bot/core/` (SQLite, `EventBus`, сервисы) и адаптеры aiogram + discord.py работают в одном процессе.

Свои функции пишете у себя на машине и подключаете через `SB_MODULES` (см. wiki [[Модули]]); в этот репозиторий они не попадают и в PR не принимаются. Сторонние модули **дополняют** процесс: встроенные пока **нельзя выключить** через env.

В комплекте — **примеры модулей** (не главная цель продукта):

| Пример в комплекте | Что делает |
|--------------------|------------|
...
| **Модерация** | Очередь, карточки, фильтры per-guild, auto-reject/flag, страйки, server automod (Discord), report/unpublish |
| **Также** | Проходка, казино, Multi-PC `/host`, operator audit |

Пример предложки: подписчик пишет боту или `/suggest` на сервере — модераторы видят карточку, одобряют или отклоняют, пост выходит в Telegram и/или Discord. Анонимность, отложенная публикация и зеркало ленты входят в этот пример. Один процесс, общая очередь модерации, dual-publish и зеркало ленты.
...
## Свой модуль (быстрый старт)

Для авторов локальных расширений — **после** клона и venv (см. [Быстрый старт](#быстрый-старт-5-минут) ниже):

1. Клонируйте репозиторий, `pip install -r requirements.txt`, подготовьте `.env` (токены — для запуска бота; для **только** проверки loader достаточно `.env` с `SB_MODULES`).
2. **Шаблон:** из корня клона — `bash scripts/modules/scaffold-local-module.sh ~/suggest-bridge-modules` или скопируйте `examples/local_module_template/hello_module.py` **вне** репозитория.
3. Укажите путь в `SB_MODULES` (не PR в этот GitHub).
4. **Validate (без полного бота):** из корня клона, venv активен — `python -m bot.core.module_loader` (проверяет путь и класс, **не** вызывает TG/DS hooks).
5. **Runtime:** перезапуск `python -m bot.main` (или Docker/systemd) → `curl http://127.0.0.1:8080/healthz` — `checks.modules` (нужен живой процесс и `HEALTH_PORT`).

Подробно: wiki **[Модули](https://github.com/noki4angel37/suggest-bridge/wiki/Модули)** · **[Добавить модуль](https://github.com/noki4angel37/suggest-bridge/wiki/Добавить-модуль)** · **[FAQ модулей](https://github.com/noki4angel37/suggest-bridge/wiki/FAQ-модулей)** · **[Add module (EN)](https://github.com/noki4angel37/suggest-bridge/wiki/Add-module-en)** · **[Module FAQ (EN)](https://github.com/noki4angel37/suggest-bridge/wiki/Module-FAQ-en)** · образцы `examples/sample_module/`, `examples/local_module_template/`.
...
- Кто хочет **открытое ядро** TG↔DS и свои функции локально, без SaaS (предложка в комплекте — пример модуля)
...
| Функция | Описание |
|---------|----------|
| Модули | Примеры в комплекте: предложка, мост, модерация, проходка, казино. Свои — локально через `SB_MODULES`, не PR в этот репозиторий |
...
Схема ниже — **пример модуля предложки** в комплекте (не единственная функция ядра):
