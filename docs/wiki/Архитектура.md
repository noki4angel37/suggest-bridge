...
Один процесс `python -m bot.main` поднимает Telegram (aiogram 3), Discord (discord.py) и планировщик отложенных публикаций — в зависимости от режима. Общее ядро: `bot/core/` (правила, сервисы, SQLite, PublishRouter, host lease). **Suggest Bridge** — модульная платформа: встроенные модули (предложка, мост, модерация, …) и опциональные через `SB_MODULES` — см. [[Модули]].
