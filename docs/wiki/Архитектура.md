...
Один процесс `python -m bot.main` поднимает Telegram (aiogram 3), Discord (discord.py) и планировщик отложенных публикаций — в зависимости от режима. Общее ядро: `bot/core/` (правила, сервисы, SQLite, PublishRouter, host lease). **Suggest Bridge** — открытое ядро: свои функции пишете локально и подключаете через `SB_MODULES` (в этот GitHub они не попадают). В комплекте — примеры (предложка, мост, модерация и др.) — см. [[Модули]].
...
| `bot/bridge.py` | Composition root: TG + DS + scheduler + lease + module loader |
| `bot/core/modules.py` | Plugin API: `BridgeModule`, `ModuleContext` |
| `bot/core/module_loader.py` | Загрузка `SB_MODULES` |
