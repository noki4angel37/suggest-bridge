...
**Предусловия:** клон [suggest-bridge](https://github.com/noki4angel37/suggest-bridge), `pip install -r requirements.txt`, `.env` (для loader достаточно `SB_MODULES`; токены — чтобы **запустить** бота).

## Быстрый старт (шаблон)

Из **корня клона** скопируйте минимальный модуль **вне** репозитория или сгенерируйте файл:
...
2. Укажите его в `.env`.
3. **Validate (без полного бота):** из корня клона, venv активен — `python -m bot.core.module_loader` — только путь и класс, **не** TG/DS hooks.
4. Перезапустите процесс бота (`python -m bot.main`, Docker `compose restart`, systemd — тот же unit, что поднимает бота).
5. **Runtime:** `curl -s http://127.0.0.1:8080/healthz` → `checks.modules` (нужен **живой** процесс и `HEALTH_PORT`). Пустой `SB_MODULES` тоже даёт `true` — убедитесь, что строка не закомментирована. `SB_MODULES_STRICT=1` — падение при битом **spec**, не при ошибке hook после загрузки.
