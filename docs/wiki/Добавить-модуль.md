...
Для Cogs используйте `await bot.add_cog(MyCog(bot, ctx))`.
...
- **События:** `ctx.bus.subscribe(DomainEventSubclass, handler)` — см. `bot/core/events.py`.
...
(путь относительно корня клона репозитория или абсолютный)
